...
function Assert-NoRunningRepoBuilds {
    ...
    $running = Get-Process -Name muteguard -ErrorAction SilentlyContinue |
    ...
    if ($running) {
        ...
        throw "Close running muteguard.exe instances before packaging to avoid file locks:`n$($paths -join "`n")"
    }
}
...
$appRoot = Join-Path $repoRoot "target\dx\muteguard\release\windows\app"
...
try {
    foreach ($target in $targets) {
        ...
        try {
            ...
            Copy-PortableApp -SourceAppDir $appRoot -PortableDir $portableDir -ZipPath $zipPath -RepoRoot $repoRoot
