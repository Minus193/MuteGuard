...
- The current 70-test Windows harness links and runs successfully. It includes
  ...
  notification XML, hotkey-action migration/idempotence, complete icon-library
  and Guide-section coverage tests.
...
- All 56 SVG files parse as XML; the web manifest parses as JSON.
...
- An anonymous read-only request to the configured public GitHub endpoint
  returned the current stable release and its expected x64 setup asset. No
  GitHub credential or token was supplied.
...
- Portable and installer archives contain no Markdown files. The packaged
  executable retains `MuteGuard` file/product descriptions and version 1.5.2.
...
Each hotkey now persists an explicit `Toggle mute`, `Mute` or `Unmute` action.
Bindings saved before 1.5.0 default to `Toggle mute`; supported legacy action
values migrate while unsupported hold/gamepad bindings remain excluded.
Explicit actions read the real state first and skip `SetMute` when the selected
endpoint already matches the request. The same path handles `--mute`,
`--unmute`, `muteguard://mute` and `muteguard://unmute`. Simultaneous duplicate
commands are coalesced deterministically without collapsing different actions.
Hotkey microphone choices include the default communications microphone, every currently
...
The `All microphones` choice retains the distinct four-square group glyph.
...
Overlay and Tray retain all 17 icon families in one uninterrupted library,
without category headings. MDI remains the default, and a selected extended
style stays visible while the library is collapsed. Both sections render the
same shared grid component and More/Less behavior.
...
- Version fields: product `MuteGuard`, file/product version `1.5.2`, original
...
| --- | ---: | --- |
| `dist/1.5.2/muteguard-1.5.2-windows-x64-portable.zip` | 8,513,687 bytes | `C21B0378EFC85C3B38571D6B5EC548BFB5AEA7BCE14A779D414B8D9617462DF5` |
| `dist/1.5.2/muteguard-1.5.2-windows-x64-setup.zip` | 6,180,537 bytes | `EFD7CC457885B0EE1C5D6E87D3F7B41A424E75F61075C4FE16A9E90691D64AAD` |
| Installer EXE (standalone and inside setup ZIP) | 6,221,959 bytes | `9C86535D44CF214469F1B891F4631D6DED063032970B3FAAFF23C40DE508D330` |
| Portable `muteguard.exe` | 20,068,352 bytes | `9FE0CEC08E90EF38C9D42FEA2FDBACC226BB2271E41D637B83D3F70DD725EE88` |
...
`muteguard-1.5.2-windows-x64-setup.zip` are present; the ZIP remains the
